...
try:
    ...
    print(f"✅ LLM Processor loaded (configured: {llm_processor.is_configured()})")
...
sessions = defaultdict(lambda: {
    ...
    'stability_threshold': 2,  # Lower = faster detection (was 3)
})
...
async def refine_text(session_id: str = Form(default="default")):
    ...
    try:
        ...
        # Use LLM if available, otherwise basic refinement
        if llm_processor and llm_processor.is_configured():
            print(f"🤖 Refining with LLM: '{sentence}'")
            refined = llm_processor.refine_text(sentence)
        else:
            print(f"📝 Using basic refinement (LLM not available)")
            refined = sentence
            if refined:
...
async def reset_session(session_id: str = Form(default="default")):
    ...
    return JSONResponse(content={'message': 'Session reset', 'session_id': session_id})
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"🚀 NodzBud Sign Language Detection API")
    print(f"{'='*60}")
